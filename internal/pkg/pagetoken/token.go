...
// EncodeUUID encodes uuid and t into base64 string separated with ",".
func EncodeUUID(pk uuid.UUID, t time.Time) string {
	s := fmt.Sprintf("%s,%s", pk, t.Format(time.RFC3339Nano))
	return base64.StdEncoding.EncodeToString([]byte(s))
}

// DecodeUUID decodes page token into uuid and created at time,
...
func DecodeUUID(token string) (uuid.UUID, time.Time, error) {
